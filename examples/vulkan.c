...
static int find_present_queue(VkPhysicalDevice device, VkSurfaceKHR surface, uint32_t *out_family) {
    uint32_t count = 0;
...
static VkExtent2D choose_extent(renderer *r, const VkSurfaceCapabilitiesKHR *caps) {
    ...
    VkExtent2D extent = {(uint32_t)(width > 0 ? width : 1), (uint32_t)(height > 0 ? height : 1)};
...
static int create_pipeline(renderer *r) {
    const VkAttachmentDescription color = {.format = r->format,
    ...
    vkDestroyShaderModule(r->device, fragment, NULL);
    if (result != VK_SUCCESS)
        vk_error("graphics pipeline creation", result);
...
static int create_swapchain(renderer *r) {
    ...
    const VkSwapchainCreateInfoKHR info = {.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
                                           ...
                                           .presentMode = choose_present_mode(r),
                                           .clipped = VK_TRUE};
    VkResult result = vkCreateSwapchainKHR(r->device, &info, NULL, &r->swapchain);
    if (result != VK_SUCCESS) {
        vk_error("vkCreateSwapchainKHR", result);
        return 0;
    }
    ...
    if (!r->images || !r->views || !r->framebuffers)
        return 0;
    ...
    for (uint32_t i = 0; i < r->image_count; ++i) {
        ...
        if (vkCreateImageView(r->device, &view_info, NULL, &r->views[i]) != VK_SUCCESS)
            return 0;
    }
    if (!create_pipeline(r))
        return 0;
    for (uint32_t i = 0; i < r->image_count; ++i) {
        const VkFramebufferCreateInfo framebuffer_info = {
            ...
            .layers = 1};
        if (vkCreateFramebuffer(r->device, &framebuffer_info, NULL, &r->framebuffers[i]) !=
            VK_SUCCESS)
            return 0;
    }
...
static int recreate_swapchain(renderer *r) {
    ...
    if (nk_window_get_framebuffer_size(r->window, &width, &height) != NK_OK || width <= 0 ||
        height <= 0)
        return 1;
...
static int record_commands(renderer *r, uint32_t image) {
    ...
    const VkRenderPassBeginInfo pass = {.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,
                                        .renderPass = r->render_pass,
                                        .framebuffer = r->framebuffers[image],
...
static int renderer_init(renderer *r) {
    if (!select_physical_device(r)) {
        fprintf(stderr, "No Vulkan device can present here.\n");
        return 0;
    }
    ...
    const VkCommandPoolCreateInfo pool = {.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO,
                                          ...
                                          .queueFamilyIndex = r->queue_family};
    if (vkCreateCommandPool(r->device, &pool, NULL, &r->command_pool) != VK_SUCCESS)
        return 0;
    ...
    if (vkAllocateCommandBuffers(r->device, &allocate, r->commands) != VK_SUCCESS)
        return 0;
    ...
    for (uint32_t i = 0; i < FRAMES_IN_FLIGHT; ++i)
        if (vkCreateSemaphore(r->device, &semaphore, NULL, &r->acquired[i]) != VK_SUCCESS ||
            ...
            vkCreateFence(r->device, &fence, NULL, &r->fences[i]) != VK_SUCCESS)
            return 0;
    return create_swapchain(r);
}
...
static void renderer_destroy(renderer *r) {
    ...
    for (uint32_t i = 0; i < FRAMES_IN_FLIGHT; ++i) {
        ...
        if (r->complete[i])
            vkDestroySemaphore(r->device, r->complete[i], NULL);
        if (r->acquired[i])
            vkDestroySemaphore(r->device, r->acquired[i], NULL);
    }
    if (r->command_pool)
...
int main(int argc, char **argv) {
    ...
    nk_window_options options = {.struct_size = sizeof(options),
    ...
    if (!extensions ||
        nk_vulkan_get_required_instance_extensions(window, extensions, &extension_count) != NK_OK)
        goto window_cleanup;
    const VkApplicationInfo application = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                                           .pApplicationName = "NativeKit Vulkan triangle",
                                           .applicationVersion = 1,
                                           .pEngineName = "NativeKit",
                                           .engineVersion = 1,
    ...
    free(extensions);
    if (result != VK_SUCCESS) {
        vk_error("vkCreateInstance", result);
        goto window_cleanup;
    }
    ...
    renderer graphics = {
        .window = window, .instance = instance, .surface = (VkSurfaceKHR)(uintptr_t)native_surface};
    if (renderer_init(&graphics)) {
        ...
        while (running) {
            ...
            do {
                ...
                if (event.source == window && event.kind == NK_EVENT_WINDOW_CLOSE)
                    running = 0;
