...
const char *method_name(nk_http_method method) {
    switch (method) {
    case NK_HTTP_METHOD_GET: return "GET";
    case NK_HTTP_METHOD_POST: return "POST";
    case NK_HTTP_METHOD_PUT: return "PUT";
    case NK_HTTP_METHOD_PATCH: return "PATCH";
    case NK_HTTP_METHOD_DELETE: return "DELETE";
    case NK_HTTP_METHOD_HEAD: return "HEAD";
    case NK_HTTP_METHOD_OPTIONS: return "OPTIONS";
    case NK_HTTP_METHOD_TRACE: return "TRACE";
    case NK_HTTP_METHOD_CONNECT: return "CONNECT";
...
EM_JS(void, start_fetch,
       (double id, const char *url, const char *method, const char *headers,
        uintptr_t body, uint32_t body_size, double max_header_size, double max_response_size,
        uint32_t redirect_limit, uint32_t allow_https_to_http), {
           ...
           let request_body = body_size === 0 ? null : HEAPU8.slice(Number(body), Number(body) + body_size);
           ...
           (async () => {
               ...
               for (;;) {
                   ...
                   if (response.status >= 300 && response.status <= 399 && location !== null) {
                       ...
                       const downgrade = current_url.substring(0, 8).toLowerCase() === 'https://' &&
...
EMSCRIPTEN_KEEPALIVE int32_t nk_net_fetch_headers(double id, int32_t status,
                                                   const char *lines, int32_t redirected) {
    try {
...
nk_result backend_start(const RequestPtr &request) noexcept {
    try {
        ...
        start_fetch(static_cast<double>(request->id), request->request.url.c_str(), method, headers.c_str(),
                    request->request.body.empty() ? 0 : reinterpret_cast<uintptr_t>(request->request.body.data()),
                    static_cast<uint32_t>(request->request.body.size()), request->client->config.max_header_size,
                    request->request.max_response_size,
