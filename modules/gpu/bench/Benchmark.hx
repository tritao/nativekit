...
class Benchmark {
	...
	static function main():Int {
		...
		for (index in 0...10000)
			warmup = NativeKitGpuBenchmark.nkgpu_benchmark_call(warmup);

		var iterations = 1000000;
		var direct = 0;
		var directStart = Date.now().getTime();
		for (index in 0...iterations)
			direct = NativeKitGpuBenchmark.nkgpu_benchmark_call(direct);
		var directMs = elapsed(directStart);

		var batchStart = Date.now().getTime();
		var batched = NativeKitGpuBenchmark.nkgpu_benchmark_batch(iterations, 0);
		...
		for (index in 0...draws)
			commandDirect = NativeKitGpuBenchmark.nkgpu_benchmark_call(commandDirect ^ index ^ 6 ^ 1);
		var commandDirectMs = elapsed(commandDirectStart);
		var commandBatchStart = Date.now().getTime();
		var commandBatch = NativeKitGpuBenchmark.nkgpu_benchmark_commands(commands,
			commands.length, 0);
