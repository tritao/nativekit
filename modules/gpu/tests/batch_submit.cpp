...
bool expect_gpu(nkgpu_result actual, nkgpu_result expected, const char *expression) {
    if (actual == expected)
        return true;
...
bool check_rgba8_readback(nkgpu_renderer renderer, nkgpu_image image,
    ...
    if (nkgpu_readback_begin_image(renderer, &desc, &request) != NKGPU_OK)
        return false;
    nkgpu_readback_info info{};
    info.struct_size = sizeof(info);
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
    while (std::chrono::steady_clock::now() < deadline) {
        if (nkgpu_readback_query(renderer, request, &info) != NKGPU_OK)
            break;
    ...
    const bool success = info.state == NKGPU_READBACK_READY && info.size == sizeof(pixel) &&
                         nkgpu_readback_read(renderer, request, pixel, sizeof(pixel), &size) ==
                             NKGPU_OK &&
                         size == sizeof(pixel) && std::memcmp(pixel, expected, sizeof(pixel)) == 0;
    nkgpu_readback_destroy(renderer, request);
    if (!success)
        std::fprintf(stderr, "sampled image readback mismatch: %u,%u,%u,%u\n", pixel[0],
...
int main() {
    ...
    /* Cube and portable cube-array images must be sampled through their
     * declared shader image types, not merely created and destroyed. */
    {
        const bool gles = nkgpu_query_graphics_api(renderer) == NK_GRAPHICS_OPENGL_ES;
        const char *vertex_source =
            gles ? "#version 300 es\n"
        ...
        const uint8_t cube_array_pixels[] = {
            ...
            160, 161, 162, 255, /* layer 6 */
            112, 113, 114, 255, 128, 129, 130, 255, 144, 145, 146, 255,
            176, 177, 178, 255, 192, 193, 194, 255,
        };
        const float vertices[] = {-1.0f, -1.0f, 3.0f, -1.0f, -1.0f, 3.0f};
        nkgpu_image cube_image{};
        nkgpu_image cube_array_image{};
        nkgpu_image target_image{};
        nkgpu_buffer vertex_buffer{};
        nkgpu_sampler cube_sampler{};
        ...
        bool success = expect_gpu(nkgpu_image_create_desc(renderer, &cube_desc, &cube_image),
        ...
        if (success)
            success = expect_gpu(nkgpu_image_create_desc(renderer, &cube_array_desc,
                                                         &cube_array_image),
        ...
        if (success)
            success = expect_gpu(nkgpu_image_create_desc(renderer, &target_desc, &target_image),
                                 NKGPU_OK, "nkgpu_image_create_desc(cube target)");
        if (success)
            success = expect_gpu(nkgpu_buffer_create(renderer,
                                                     reinterpret_cast<const uint8_t *>(vertices),
                                                     sizeof(vertices), &vertex_buffer),
                                 NKGPU_OK, "nkgpu_buffer_create(cube vertices)");
        if (success)
            success = expect_gpu(nkgpu_sampler_create(renderer, NKGPU_FILTER_NEAREST,
                                                      NKGPU_FILTER_NEAREST,
                                                      NKGPU_WRAP_CLAMP_TO_EDGE,
                                                      NKGPU_WRAP_CLAMP_TO_EDGE, &cube_sampler),
                                 NKGPU_OK, "nkgpu_sampler_create(cube)");

        auto create_sample_pipeline = [&](const char *fragment_source, nkgpu_image_type type,
                                          nkgpu_shader *out_shader,
                                          nkgpu_pipeline *out_pipeline) -> bool {
            nkgpu_shader_builder builder{};
            if (!expect_gpu(nkgpu_shader_begin(renderer, NKGPU_SHADERLANGUAGE_GLSL,
                                               vertex_source, fragment_source, &builder),
            ...
            image_binding.image_type = type;
            if (!expect_gpu(nkgpu_shader_binding(builder, &image_binding), NKGPU_OK,
                            "nkgpu_shader_binding(cube image)"))
                return false;
            nkgpu_shader_binding_desc sampler_binding = image_binding;
            sampler_binding.kind = NKGPU_SHADERBINDING_SAMPLER;
            if (!expect_gpu(nkgpu_shader_binding(builder, &sampler_binding), NKGPU_OK,
                            "nkgpu_shader_binding(cube sampler)"))
                return false;
            if (!expect_gpu(nkgpu_shader_end(builder, out_shader), NKGPU_OK,
                            "nkgpu_shader_end(cube)"))
                return false;
            nkgpu_pipeline_builder pipeline_builder{};
            if (!expect_gpu(nkgpu_pipeline_begin(renderer, *out_shader, 2 * sizeof(float),
                                                 &pipeline_builder),
                            NKGPU_OK, "nkgpu_pipeline_begin(cube)"))
                return false;
            if (!expect_gpu(nkgpu_pipeline_attribute(pipeline_builder, 0, 0, 0,
                                                     NKGPU_VERTEXFORMAT_FLOAT2),
                            NKGPU_OK, "nkgpu_pipeline_attribute(cube)"))
                return false;
            if (!expect_gpu(nkgpu_pipeline_color_target(pipeline_builder, 0,
                                                        NKGPU_IMAGEFORMAT_RGBA8,
                                                        NKGPU_COLORMASK_RGBA, nullptr),
                            NKGPU_OK, "nkgpu_pipeline_color_target(cube)"))
                return false;
            if (!expect_gpu(nkgpu_pipeline_depth_stencil(pipeline_builder, 0), NKGPU_OK,
                            "nkgpu_pipeline_depth_stencil(cube)"))
                return false;
            return expect_gpu(nkgpu_pipeline_end(pipeline_builder, out_pipeline), NKGPU_OK,
                              "nkgpu_pipeline_end(cube)");
        };

        if (success)
            success = create_sample_pipeline(cube_fragment, NKGPU_IMAGETYPE_CUBE, &cube_shader,
                                             &cube_pipeline);
        if (success)
            success = create_sample_pipeline(cube_array_fragment, NKGPU_IMAGETYPE_CUBE_ARRAY,
                                             &cube_array_shader, &cube_array_pipeline);

        auto sample_image = [&](nkgpu_pipeline pipeline, nkgpu_image image,
                                const uint8_t expected[4]) -> bool {
            nkgpu_render_pass_desc pass{};
            pass.struct_size = sizeof(pass);
            pass.color_count = 1;
            pass.colors[0].image = target_image;
            ...
            bool sampled = expect_gpu(nkgpu_frame_begin(renderer), NKGPU_OK,
                                      "nkgpu_frame_begin(cube sample)") &&
                           expect_gpu(nkgpu_begin_render_pass(renderer, &pass), NKGPU_OK,
                                      "nkgpu_begin_render_pass(cube sample)") &&
                           expect_gpu(nkgpu_apply_pipeline(renderer, pipeline), NKGPU_OK,
                                      "nkgpu_apply_pipeline(cube sample)") &&
                           expect_gpu(nkgpu_apply_vertex_buffer(renderer, 0, vertex_buffer, 0),
                                      NKGPU_OK, "nkgpu_apply_vertex_buffer(cube sample)") &&
                           expect_gpu(nkgpu_apply_image(renderer, 0, image), NKGPU_OK,
                                      "nkgpu_apply_image(cube sample)") &&
                           expect_gpu(nkgpu_apply_sampler(renderer, 0, cube_sampler), NKGPU_OK,
                                      "nkgpu_apply_sampler(cube sample)") &&
                           expect_gpu(nkgpu_draw(renderer, 0, 3, 1), NKGPU_OK,
                                      "nkgpu_draw(cube sample)") &&
                           expect_gpu(nkgpu_end_pass(renderer), NKGPU_OK,
                                      "nkgpu_end_pass(cube sample)") &&
                           expect_gpu(nkgpu_end_frame(renderer), NKGPU_OK,
                                      "nkgpu_end_frame(cube sample)");
            return sampled && check_rgba8_readback(renderer, target_image, expected);
        };
        const uint8_t cube_expected[] = {255, 0, 0, 255};
        const uint8_t cube_array_expected[] = {160, 161, 162, 255};
        if (success)
            success = sample_image(cube_pipeline, cube_image, cube_expected);
        if (success)
            success = sample_image(cube_array_pipeline, cube_array_image, cube_array_expected);

        if (cube_array_pipeline.id)
            expect_gpu(nkgpu_pipeline_destroy(renderer, cube_array_pipeline), NKGPU_OK,
                       "nkgpu_pipeline_destroy(cube array)");
        if (cube_pipeline.id)
            expect_gpu(nkgpu_pipeline_destroy(renderer, cube_pipeline), NKGPU_OK,
                       "nkgpu_pipeline_destroy(cube)");
        if (cube_array_shader.id)
            expect_gpu(nkgpu_shader_destroy(renderer, cube_array_shader), NKGPU_OK,
                       "nkgpu_shader_destroy(cube array)");
        if (cube_shader.id)
            expect_gpu(nkgpu_shader_destroy(renderer, cube_shader), NKGPU_OK,
                       "nkgpu_shader_destroy(cube)");
        if (cube_sampler.id)
            expect_gpu(nkgpu_sampler_destroy(renderer, cube_sampler), NKGPU_OK,
                       "nkgpu_sampler_destroy(cube)");
        if (vertex_buffer.id)
            expect_gpu(nkgpu_buffer_destroy(renderer, vertex_buffer), NKGPU_OK,
                       "nkgpu_buffer_destroy(cube vertices)");
        if (target_image.id)
            expect_gpu(nkgpu_image_destroy(renderer, target_image), NKGPU_OK,
                       "nkgpu_image_destroy(cube target)");
        if (cube_array_image.id)
            expect_gpu(nkgpu_image_destroy(renderer, cube_array_image), NKGPU_OK,
                       "nkgpu_image_destroy(cube array)");
        if (cube_image.id)
            expect_gpu(nkgpu_image_destroy(renderer, cube_image), NKGPU_OK,
                       "nkgpu_image_destroy(cube)");
        if (!success) {
            result = __LINE__;
            goto cleanup;
        }
