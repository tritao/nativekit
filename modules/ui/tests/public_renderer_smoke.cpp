...
nkui_layer_command make_layer(float opacity, float x = 0.0f, float y = 0.0f, float width = 0.0f,
                              float height = 0.0f, nkui_layer_flags flags = 0) {
...
void append_layer(std::vector<uint8_t> &bytes, nkui_layer_command layer,
    ...
    layer.header.size = static_cast<uint32_t>(
        sizeof(layer) + (foreground.size() + backdrop.size()) * sizeof(nkui_effect_op_command));
    append(bytes, layer);
...
int main(int argc, char **argv) {
    ...
    if (surface_options.api == NK_GRAPHICS_OPENGL) {
        static constexpr char custom_glsl410[] =
            "#version 410\n"
            "uniform vec4 effect_fs_params[5];\n"
            "uniform sampler2D tex_smp;\n"
            "layout(location = 0) in vec2 uv;\n"
            "layout(location = 0) out vec4 frag_color;\n"
            "void main() {\n"
            "    vec4 source = texture(tex_smp, uv);\n"
            "    frag_color = source * effect_fs_params[0].x;\n"
            "}\n";
        static constexpr char custom_glsl300es[] =
            "#version 300 es\n"
            ...
            "    frag_color = source * effect_fs_params[0].x;\n"
            "}\n";
        nkui_custom_effect_registration registration{};
        registration.struct_size = sizeof(registration);
        registration.registration_id = 77;
        registration.name = "smoke-multiply";
        registration.glsl410_fragment = custom_glsl410;
        registration.glsl300es_fragment = custom_glsl300es;
        registration.parameter_components = 1;
        registration.pass_count = 1;
        registration.sampling_inputs = 1;
        if (nkui_renderer_register_custom_effect(renderer, &registration) != NKUI_OK)
            return 5;
        custom_effect_registered = true;
    }
    ...
    append(commands, nkui_command_header{NKUI_COMMAND_END_LAYER, NKUI_COMMAND_VERSION,
                                         sizeof(nkui_command_header)});
    auto blur_layer =
        make_layer(1.0f, 72.0f, 82.0f, 112.0f, 70.0f, NKUI_LAYER_ISOLATED | NKUI_LAYER_HAS_BOUNDS);
    ...
    append(box_shadow_commands,
           nkui_scalar_command{
               {NKUI_COMMAND_SET_GLOBAL_ALPHA, NKUI_COMMAND_VERSION, sizeof(nkui_scalar_command)},
               0.78f});
    append(box_shadow_commands, nkui_rect_command{{NKUI_COMMAND_CLIP_RECT, NKUI_COMMAND_VERSION,
    ...
    nkui_draw_box_shadow_command box_shadow_command{
        {NKUI_COMMAND_DRAW_BOX_SHADOW, NKUI_COMMAND_VERSION, sizeof(nkui_draw_box_shadow_command)},
        34.0f,
        34.0f,
        92.0f,
        58.0f,
