            ...
            while (running) {
                ...
                switch (value) {
                    ...
                    case SurfaceReady(source) if (source == surface):
                        ...
                        if (size.status != NativeKitConstants.NK_OK)
                        ...
                        if (windowScale.status != NativeKitConstants.NK_OK)
                    ...
                    case PointerButton(source, _, action, _, x, y) if (source == window):
                        app.pointerButton(x, y, action == NativeKitConstants.NK_INPUT_PRESS);
                    case Key(source, key, _, action, _) if (source == window &&
                        action == NativeKitConstants.NK_INPUT_PRESS && key == NativeKitConstants.NK_KEY_ESCAPE):
                        running = false;
                    default:
                }
