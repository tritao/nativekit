...
artifact="$build_dir/haxeon-ui-text-editor-benchmark.hl"
...
if [[ -x "$haxeon_dir/scripts/build-runtime.sh" ]]; then
	...
	echo "benchmark-haxeon-text-editor: Haxeon runtime build script not found" >&2
	exit 2
fi
...
if [[ ! -x "$hashlink_runtime" ]]; then
	echo "benchmark-haxeon-text-editor: missing HashLink runtime" >&2
	exit 2
fi

(cd "$haxeon_dir" && .tools/haxe/haxe -cp src --run compiler.tools.HaxeonCompiler \
	--output="$artifact" \
	--entry=TextEditorBenchmark \
	...
	"$module_dir/tests/haxeon/TextEditorBenchmark.hx" \
