...
artifact="$build_dir/haxeon-ui-offset-map-benchmark.hl"
...
if [[ -x "$haxeon_dir/scripts/build-runtime.sh" ]]; then
	...
	echo "benchmark-haxeon-offset-map: Haxeon runtime build script not found" >&2
	exit 2
fi
...
if [[ ! -x "$hashlink_runtime" ]]; then
	echo "benchmark-haxeon-offset-map: missing HashLink runtime" >&2
	exit 2
fi

(cd "$haxeon_dir" && .tools/haxe/haxe -cp src --run compiler.tools.HaxeonCompiler \
	--output="$artifact" \
	--entry=TextOffsetMapBenchmark \
	...
	"$module_dir/tests/haxeon/TextOffsetMapBenchmark.hx" \
