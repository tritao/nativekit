...
constexpr mask k_known_capabilities =
    ...
    cap(NK_CAP_WINDOW_CUSTOM_DECORATIONS) | cap(NK_CAP_NATIVE_VIEW);
...
constexpr backend_contract platform_contract() {
    ...
    return {"Linux/GTK",
            k_desktop_common | cap(NK_CAP_WEBVIEW) | cap(NK_CAP_OPENGL_SURFACE) |
                ...
                cap(NK_CAP_NATIVE_VIEW) | k_linux_system_capabilities,
...
constexpr std::array<capability_name, 38> k_capability_names = {{
