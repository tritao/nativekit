...
constexpr mask k_known_capabilities =
    cap(NK_CAP_WINDOW) | cap(NK_CAP_WEBVIEW) | cap(NK_CAP_FILE_DIALOG) |
...
constexpr mask k_desktop_common = cap(NK_CAP_WINDOW) | cap(NK_CAP_FILE_DIALOG) |
                                   cap(NK_CAP_CLIPBOARD) | cap(NK_CAP_DRAG_DROP) |
                                   cap(NK_CAP_SHELL) | cap(NK_CAP_SYSTEM_APPEARANCE) |
                                   cap(NK_CAP_EXPORT_NATIVE_WINDOW) | cap(NK_CAP_NOTIFICATION) |
                                   cap(NK_CAP_INPUT) | cap(NK_CAP_CURSOR) |
...
constexpr backend_contract current_contract() {
    ...
    return {"Windows",
            k_desktop_common | cap(NK_CAP_D3D11_SURFACE) | cap(NK_CAP_ACCESSIBILITY),
    ...
    return {"macOS",
            k_desktop_common | cap(NK_CAP_WEBVIEW) | cap(NK_CAP_METAL_SURFACE),
            cap(NK_CAP_WINDOW_GEOMETRY) | cap(NK_CAP_WINDOW_STYLING) | cap(NK_CAP_MONITOR) |
                cap(NK_CAP_MONITOR_FULLSCREEN) | cap(NK_CAP_JOYSTICK) |
    ...
    return {"iOS",
            cap(NK_CAP_MOBILE_HOST) | cap(NK_CAP_RESOURCE_IO),
            cap(NK_CAP_WEBVIEW) | cap(NK_CAP_FILE_DIALOG) | cap(NK_CAP_CLIPBOARD) |
                ...
                cap(NK_CAP_NOTIFICATION) | cap(NK_CAP_INPUT) | cap(NK_CAP_METAL_SURFACE) |
                cap(NK_CAP_RESOURCE_SHARING) | cap(NK_CAP_JOYSTICK) | cap(NK_CAP_ACCESSIBILITY),
            cap(NK_CAP_WINDOW) | cap(NK_CAP_EXPORT_NATIVE_WINDOW) | cap(NK_CAP_WRAP_NATIVE_WINDOW) |
                cap(NK_CAP_WINDOW_GEOMETRY) | cap(NK_CAP_WINDOW_STYLING) | cap(NK_CAP_MONITOR) |
                cap(NK_CAP_MONITOR_FULLSCREEN) | cap(NK_CAP_CURSOR) | cap(NK_CAP_POINTER_CAPTURE) |
                cap(NK_CAP_OPENGL_SURFACE) | cap(NK_CAP_OPENGL_ES_SURFACE) |
    ...
    return {"Web",
            cap(NK_CAP_WINDOW) | cap(NK_CAP_INPUT) | cap(NK_CAP_OPENGL_ES_SURFACE) |
                ...
                cap(NK_CAP_WINDOW_GEOMETRY) | cap(NK_CAP_RESOURCE_IO),
            ...
            0};
#else
    return {"fallback stub", cap(NK_CAP_RESOURCE_IO), k_known_capabilities & ~cap(NK_CAP_RESOURCE_IO), 0,
            0};
#endif
}

bool check_disjoint(const backend_contract &contract) {
    const mask categories = contract.required | contract.deferred | contract.not_applicable |
                            contract.optional;
    const mask overlap = (contract.required & contract.deferred) |
                         (contract.required & contract.not_applicable) |
                         (contract.required & contract.optional) |
                         (contract.deferred & contract.not_applicable) |
                         (contract.deferred & contract.optional) |
    ...
    if (categories != k_known_capabilities) {
        std::fprintf(stderr, "%s contract does not classify every capability bit: 0x%llx\n",
                     contract.name, static_cast<unsigned long long>(k_known_capabilities ^ categories));
        return false;
    }
    if (overlap != 0) {
