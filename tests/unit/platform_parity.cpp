...
constexpr backend_contract platform_contract() {
    ...
    return {
        ...
        cap(NK_CAP_WEBVIEW) | cap(NK_CAP_MOBILE_HOST) | cap(NK_CAP_EXPORT_NATIVE_WINDOW) |
            cap(NK_CAP_WRAP_NATIVE_WINDOW) | cap(NK_CAP_MONITOR) | cap(NK_CAP_MONITOR_FULLSCREEN) |
            cap(NK_CAP_OPENGL_SURFACE) | cap(NK_CAP_VULKAN_SURFACE) | cap(NK_CAP_D3D11_SURFACE) |
            cap(NK_CAP_METAL_SURFACE) | cap(NK_CAP_WINDOW_CUSTOM_DECORATIONS),
        cap(NK_CAP_SYSTEM_APPEARANCE) | cap(NK_CAP_KEEP_AWAKE) | cap(NK_CAP_DISPLAY_ORIENTATION)};
#else
