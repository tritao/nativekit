...
| Web | Window, geometry, clipboard, input, cursor/capture, GLES, resource I/O, accessibility, platform identity, locale, appearance, keep-awake, display orientation where browser APIs are available | Styling, dialogs, drag/drop, shell, notifications, resource sharing, joystick, application paths, application storage, system fonts, device orientation |
