...
import NativeKit.NkTextEditAction;
...
class NativeKitEventDecoderTests {
	public static function run():Bool {
		...
		var none = new NativeKitEventContext(NkEventKind.None, 0, zero, 0, 0, 0, empty);
		...
		var shortKey = new NativeKitEventContext(NkEventKind.Key, 0, zero, 0, 0, 0, haxe.io.Bytes.alloc(15));
		var shortWindow = new NativeKitEventContext(NkEventKind.WindowResize, 0, zero, 0, 0, 0, haxe.io.Bytes.alloc(7));
		var shortEdit = new NativeKitEventContext(NkEventKind.TextEdit, 0, zero, 0, 0, 0, haxe.io.Bytes.alloc(47));
		var pathsPayload = haxe.io.Bytes.alloc(16);
		putU32(pathsPayload, 8, 16); putU32(pathsPayload, 12, 16);
		var pathsContext = new NativeKitEventContext(NkEventKind.DialogPathsComplete, 0, zero, 0, 1, 0, pathsPayload);
		...
		var messageContext = new NativeKitEventContext(NkEventKind.DialogMessageComplete, 0, zero, 0, 4, 0, messagePayload);
		...
		var resourcesContext = new NativeKitEventContext(NkEventKind.DialogResourcesComplete, 0, zero, 0, 5, 0, resourcesPayload);
		var resourcesOk = switch NativeKitEvent.decodeContext(resourcesContext) {
			case Resources(kind, _, result, accepted, items): kind == NkEventKind.DialogResourcesComplete && result == 0 && !accepted && items.length == 0;
			case _: false;
		};
		var editPayload = haxe.io.Bytes.alloc(50);
		putU32(editPayload, 0, NkTextEditAction.Compose);
		...
		var editContext = new NativeKitEventContext(NkEventKind.TextEdit, 9, zero, 0, 0, 0, editPayload);
		var editOk = switch NativeKitEvent.decodeContext(editContext) {
			case TextEdit(9, edit): edit.action == NkTextEditAction.Compose && edit.text == "é" && edit.replaceStart == 1 && edit.compositionEnd == 2;
			case _: false;
		};
