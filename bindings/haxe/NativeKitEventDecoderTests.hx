...
import NativeKit.Nk_text_edit_action;
...
class NativeKitEventDecoderTests {
	public static function run():Bool {
		...
		var none = new NativeKitEventContext(Nk_event_kind.NK_EVENT_NONE, 0, zero, 0, 0, 0, empty);
		...
		var shortKey = new NativeKitEventContext(Nk_event_kind.NK_EVENT_KEY, 0, zero, 0, 0, 0, haxe.io.Bytes.alloc(15));
		var shortWindow = new NativeKitEventContext(Nk_event_kind.NK_EVENT_WINDOW_RESIZE, 0, zero, 0, 0, 0, haxe.io.Bytes.alloc(7));
		var shortEdit = new NativeKitEventContext(Nk_event_kind.NK_EVENT_TEXT_EDIT, 0, zero, 0, 0, 0, haxe.io.Bytes.alloc(47));
		var pathsPayload = haxe.io.Bytes.alloc(16);
		putU32(pathsPayload, 8, 16); putU32(pathsPayload, 12, 16);
		var pathsContext = new NativeKitEventContext(Nk_event_kind.NK_EVENT_DIALOG_PATHS_COMPLETE, 0, zero, 0, 1, 0, pathsPayload);
		...
		var messageContext = new NativeKitEventContext(Nk_event_kind.NK_EVENT_DIALOG_MESSAGE_COMPLETE, 0, zero, 0, 4, 0, messagePayload);
		...
		var resourcesContext = new NativeKitEventContext(Nk_event_kind.NK_EVENT_DIALOG_RESOURCES_COMPLETE, 0, zero, 0, 5, 0, resourcesPayload);
		var resourcesOk = switch NativeKitEvent.decodeContext(resourcesContext) {
			case Resources(kind, _, result, accepted, items): kind == Nk_event_kind.NK_EVENT_DIALOG_RESOURCES_COMPLETE && result == 0 && !accepted && items.length == 0;
			case _: false;
		};
		var editPayload = haxe.io.Bytes.alloc(50);
		putU32(editPayload, 0, Nk_text_edit_action.NK_TEXT_EDIT_COMPOSE);
		...
		var editContext = new NativeKitEventContext(Nk_event_kind.NK_EVENT_TEXT_EDIT, 9, zero, 0, 0, 0, editPayload);
		var editOk = switch NativeKitEvent.decodeContext(editContext) {
			case TextEdit(9, edit): edit.action == Nk_text_edit_action.NK_TEXT_EDIT_COMPOSE && edit.text == "é" && edit.replaceStart == 1 && edit.compositionEnd == 2;
			case _: false;
		};
