...
namespace {
nk_result string_at(const nk_event *event, uint32_t count, uint32_t strings_offset, uint32_t index,
...
nk_result NK_CALL nk_clipboard_event_file(const nk_event *event, uint32_t index,
    ...
    if (!event || event->kind != NK_EVENT_CLIPBOARD_FILES_COMPLETE ||
        event->data_size < sizeof(nk_clipboard_files))
        return NK_ERROR_INVALID_ARGUMENT;
    nk_clipboard_files header{};
    std::memcpy(&header, event->data, sizeof(header));
    return string_at(event, header.path_count, header.strings_offset, index, out_path, out_length);
}

nk_result NK_CALL nk_drop_event_item(const nk_event *event, uint32_t index, const char **out_item,
                                     uint32_t *out_length) {
    nk::core::clear_error();
    if (!event || (event->kind != NK_EVENT_DROP_FILES && event->kind != NK_EVENT_DROP_TEXT) ||
        event->data_size < sizeof(nk_drop_data))
        return NK_ERROR_INVALID_ARGUMENT;
    nk_drop_data header{};
    std::memcpy(&header, event->data, sizeof(header));
    return string_at(event, header.item_count, header.strings_offset, index, out_item, out_length);
}
}
