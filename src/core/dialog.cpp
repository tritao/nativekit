...
extern "C" nk_result NK_CALL nk_dialog_event_path(
    ...
    if (index >= payload->path_count || payload->offsets_offset > size ||
        offset_bytes > size - payload->offsets_offset ||
        payload->strings_offset > size ||
    ...
    uint32_t offset = 0;
    std::memcpy(&offset,
                bytes + payload->offsets_offset + index * sizeof(uint32_t),
                sizeof(offset));
