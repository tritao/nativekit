...
nk_handle HandleRegistry::insert(ResourceType type, std::shared_ptr<Resource> resource) {
    if (type == ResourceType::none || !resource) return NK_INVALID_HANDLE;
    ...
    if (slots_.size() >= index_mask) return NK_INVALID_HANDLE;
...
std::shared_ptr<Resource> HandleRegistry::get(nk_handle handle, ResourceType type) const {
    if (handle == NK_INVALID_HANDLE || (handle & index_mask) == 0) return {};
    std::lock_guard lock(mutex_);
    const auto index = decode_index(handle);
    if (index >= slots_.size()) return {};
    const auto& slot = slots_[index];
    if (slot.generation != decode_generation(handle) || slot.type != type) return {};
    return slot.resource;
}

bool HandleRegistry::erase(nk_handle handle, ResourceType type) {
    if (handle == NK_INVALID_HANDLE || (handle & index_mask) == 0) return false;
    std::lock_guard lock(mutex_);
    const auto index = decode_index(handle);
    if (index >= slots_.size()) return false;
    auto& slot = slots_[index];
    if (!slot.resource || slot.generation != decode_generation(handle) || slot.type != type) return false;
    slot.resource.reset();
