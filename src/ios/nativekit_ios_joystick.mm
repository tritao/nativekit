...
struct IOSJoystick final : nk::core::Resource {
...
void update(IOSJoystick &device) {
...
std::shared_ptr<IOSJoystick> lookup(nk_handle handle) {
    return std::dynamic_pointer_cast<IOSJoystick>(
...
void add_device(GCController *controller) {
    ...
    if (devices.find(key) != devices.end())
        return;
    auto device = std::make_shared<IOSJoystick>();
