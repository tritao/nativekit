...
nk_result NK_CALL nk_frame_backend_submit(const nk_surface_frame_target *target) {
    ...
    auto *swapchain =
        reinterpret_cast<IDXGISwapChain1 *>(static_cast<uintptr_t>(target->native_present_target));
...
nk_result NK_CALL nk_graphics_bind_frame_target(const nk_surface_frame_target *target) {
    if (!target || target->api != NK_GRAPHICS_D3D11 || !target->native_device ||
...
nk_result NK_CALL nk_frame_backend_cancel(nk_handle handle, const nk_surface_frame_target *target) {
    return nk_frame_backend_finish(handle, target);
}
