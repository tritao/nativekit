...
- (void)scrollWheel:(NSEvent *)event {
    ...
    nk::core::callback_boundary([&] {
        ...
        const double x =
            (event.hasPreciseScrollingDeltas ? event.scrollingDeltaX : event.deltaX) * scale;
        const double y =
            (event.hasPreciseScrollingDeltas ? -event.scrollingDeltaY : -event.deltaY) * scale;
...
- (NSRect)firstRectForCharacterRange:(NSRange)range actualRange:(NSRangePointer)actualRange {
    ...
    const nk_text_input_range_rect *range_rect = nullptr;
    ...
    NSRect rect;
    if (range_rect) {
        rect = NSMakeRect(range_rect->x, range_rect->y, std::max(1.f, range_rect->width),
                          std::max(1.f, range_rect->height));
    } else if (rectangles && !rectangles->empty()) {
...
nk_capabilities NK_CALL nk_get_capabilities(void) {
    return NK_CAP_WINDOW | NK_CAP_CLIPBOARD | NK_CAP_WEBVIEW | NK_CAP_DRAG_DROP | NK_CAP_SHELL |
           ...
           NK_CAP_WINDOW_CUSTOM_DECORATIONS | NK_CAP_GAMEPAD_RUMBLE | NK_CAP_APPLICATION_MENU |
